...
func (err errConnectionFailed) Error() string {
	if err.host == "" {
		return "Cannot connect to the Malice daemon. Is the malice daemon running on this host?"
	}
	return fmt.Sprintf("Cannot connect to the Malice daemon at %s. Is the malice daemon running?", err.host)
}
...
// ErrorConnectionFailed returns an error with host in the error message when connection to malice daemon failed.
...
// object (image, container, network, volume, …) is not found in the malice host.
func IsErrNotFound(err error) bool {
	te, ok := err.(notFound)
	return ok && te.NotFound()
}

// imageNotFoundError implements an error returned when an image is not in the malice host.
...
// when an image is not found in the malice host.
func IsErrImageNotFound(err error) bool {
	return IsErrNotFound(err)
}

// containerNotFoundError implements an error returned when a container is not in the malice host.
...
// when a container is not found in the malice host.
func IsErrContainerNotFound(err error) bool {
	return IsErrNotFound(err)
}

// networkNotFoundError implements an error returned when a network is not in the malice host.
...
// when a network is not found in the malice host.
func IsErrNetworkNotFound(err error) bool {
	return IsErrNotFound(err)
}

// volumeNotFoundError implements an error returned when a volume is not in the malice host.
...
// when a volume is not found in the malice host.
...
func (cli *Client) NewVersionError(APIrequired, feature string) error {
	if versions.LessThan(cli.version, APIrequired) {
		return fmt.Errorf("%q requires API version %s, but the Malice daemon API version is %s", feature, APIrequired, cli.version)
	}
	return nil
}
...
// pluginNotFoundError implements an error returned when a plugin is not in the malice host.
...
// when a plugin is not found in the malice host.
